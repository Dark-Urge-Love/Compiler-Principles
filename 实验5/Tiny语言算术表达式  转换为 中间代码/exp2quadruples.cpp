...
enum TokenID {ID,NUMBER,ADD,SUB,MUL,DIV,LBRACKET,RBRACKET,ENDINPUT };
...
OperandsNode factor()
{
  ...
  switch(token.ID)
  {
    case LBRACKET:  match(LBRACKET);
				    ...
				    match(RBRACKET);
	                break;
	case NUMBER:    x.flag=0; x.val=token.val;  // 产生一个数值操作数
					match(NUMBER); 
					break;
	case ID:        x.flag=1; strcpy(x.NewVar,token.word);  // 产生一个标识符操作数
					match(ID); 
					break;
